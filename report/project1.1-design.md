...
```C
#define MAX_ARGS = 30; // Controls the max args poosible for the progarm.
```
...
برای پیاده‌سازی wait باید از مکانیزم  lock استفاده کنیم. به این صورت که ابتدا پردازه فرزندی را که pid آن به عنوان پارامتر به تابع پاس داده شده است را پیدا می‌کنیم (اگر وجود نداشت باید -۱ برگردانیم.)
حال باید صبر کنیم تا وقتی که مقدار semaphore ما به ۱ برسد (که یعنی کار پردازه‌ی فرزند تمام شده و exit code آن آماده است.)
در نهایت نیز وقتی که کار پردازه فرزند تمام شد باید حافظه‌ای را که استفاده کرده بود free کنیم.
